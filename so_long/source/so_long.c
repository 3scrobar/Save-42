...
/*   Updated: 2024/12/19 17:59:09 by tle-saut         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int key_press(int keycode, t_all *data)
{
	...
	if (keycode == 65307)
		close_window(data);
	else if (keycode == 119  && check_coll(data, "haut") != 1) // W
		data->ybegin -= 1;
	else if (keycode == 115  && check_coll(data, "bas") != 1) // S
		data->ybegin += 1;
	else if (keycode == 97  && check_coll(data, "left") != 1) // A
		data->xbegin -= 1;
	else if (keycode == 100 && check_coll(data, "right") != 1) // D
		data->xbegin += 1;
	return (0);
}
